...
def _module_execution_device(module, fallback=None):
    ...
    saw_parameter = False
    try:
        saw_parameter = next(module.parameters(), None) is not None
    except Exception:
        saw_parameter = False
...
class KDiffusionStableDiffusionXLPipeline(StableDiffusionXLImg2ImgPipeline):
    ...
    def encode_cropped_prompt_77tokens(self, prompt: str):
        target_device = _module_execution_device(
            self.unet,
        ...
        for tokenizer, text_encoder in zip(tokenizers, text_encoders):
            ...
            input_device = _module_execution_device(text_encoder, target_device)

            prompt_embeds = text_encoder(text_input_ids.to(input_device), output_hidden_states=True, return_dict=False)

            # We are only ALWAYS interested in the pooled output of the final text encoder
            pooled_prompt_embeds = prompt_embeds[0].to(target_device)
            prompt_embeds = prompt_embeds[-1][-2].to(target_device)
            bs_embed, seq_len, _ = prompt_embeds.shape
            prompt_embeds = prompt_embeds.view(bs_embed, seq_len, -1)
            prompt_embeds_list.append(prompt_embeds)

        prompt_embeds = torch.concat(prompt_embeds_list, dim=-1).to(dtype=self.unet.dtype, device=target_device)
    ...
    def __call__(
        ...
        device = _module_execution_device(
            self.unet,
            torch.device("cuda" if torch.cuda.is_available() else "cpu"),
        )
        ...
        if fullpage is not None:
            vae_device = _module_execution_device(self.vae, device)
            ...
            c_concat = img2tensor(c_concat, normalize=True).to(device=vae_device, dtype=self.vae.dtype)
        ...
        vae_device = _module_execution_device(self.vae, device)
        latents = latents.to(dtype=self.trans_vae.dtype, device=vae_device) / self.vae.config.scaling_factor
