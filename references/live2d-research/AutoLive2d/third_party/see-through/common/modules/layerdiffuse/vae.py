...
class TransparentVAEDecoder(torch.nn.Module):
    ...
    def forward(self, sd_vae, latent, return_type='numpy', rgb_cond=None, mask=None, return_rgb=False):
        vae_device = _module_execution_device(sd_vae, latent.device)
        decoder_device = _module_execution_device(self, vae_device)
        try:
            current_decoder_device = next(self.parameters()).device
            if current_decoder_device != decoder_device:
                self.to(device=decoder_device)
        except Exception:
            pass

        latent = latent.to(device=vae_device, dtype=sd_vae.dtype)
        pixel = sd_vae.decode(latent).sample
        pixel = (pixel * 0.5 + 0.5).to(device=decoder_device, dtype=self.dtype)
        latent = latent.to(device=decoder_device, dtype=self.dtype)
        pixel = pixel.to(latent)
        if mask is not None:
            mask = mask.to(device=decoder_device, dtype=self.dtype)
        result_list = []
        vis_list = []
...
class TransparentVAEEncoder(torch.nn.Module):
    ...
    def forward(self, sd_vae, list_of_np_rgba_hwc_uint8, use_offset=True):
        ...
        vae_feed = vae_feed.to(device=vae_device, dtype=sd_vae.dtype)
        latent_dist = sd_vae.encode(vae_feed).latent_dist
        offset_feed = torch.cat([a_bchw_01, rgb_padded_bchw_01], dim=1).to(
            device=_module_execution_device(self, vae_device),
            dtype=self.dtype,
        )
        offset = self.model(offset_feed) * self.alpha
        if use_offset:
...
def vae_encode(vae, trans_vae_encoder: TransparentVAEEncoder, argb_tensor: torch.Tensor, use_offset=True, scale=True) -> torch.Tensor:
    vae_device = _module_execution_device(vae, argb_tensor.device)
    latent = trans_vae_encoder.encode(argb_tensor.to(dtype=vae.dtype, device=vae_device), vae, use_offset=use_offset)
    if scale:
