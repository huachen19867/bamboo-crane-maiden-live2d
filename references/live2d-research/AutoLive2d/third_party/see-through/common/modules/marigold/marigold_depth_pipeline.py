...
def _module_execution_device(module, fallback=None):
    hook = getattr(module, "_hf_hook", None)
    device = _as_torch_device(getattr(hook, "execution_device", None))
    ...
    device = _as_torch_device(getattr(module, "device", None))
    if device is not None and device.type not in {"cpu", "meta"}:
        return device

    if fallback is not None:
        return _as_torch_device(fallback)
...
def encode_argb_list(vae, batched_img_list, dtype=torch.float32, pad_argb=False):
    batched_latent_list = []
    vae_device = _module_execution_device(vae)
    for img_list in batched_img_list:
        if pad_argb:
            img_list = pad_rgb_torch(img_list.float(), return_format='argb')
        img_list = img_list.to(device=vae_device, dtype=vae.dtype)
...
def encode_depth_list(vae, batched_depth_list, dtype=torch.float32):
    ...
    for depth_list in batched_depth_list:
        depth_list = depth_list.to(device=vae_device, dtype=vae.dtype)
        batched_latent_list.append(
...
class MarigoldDepthPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs
        ...
        if img_list is not None:
            ...
            with torch.no_grad():
                vae = self.vae
                vae_device = _module_execution_device(vae, _module_execution_device(self.unet))
                rgb_latent = [encode_argb_list(vae, img[None, None].to(device=vae_device, dtype=vae.dtype), pad_argb=True, dtype=vae.dtype) for img in img_list_tensor]
        ...
        if cond_latent is not None:
            ...
            rgb_latent = self.encode_rgb(rgb_norm.to(dtype=self.vae.dtype, device=_module_execution_device(self.vae, _module_execution_device(self.unet))))
    ...
    def encode_empty_text(self):
        ...
        text_input_ids = text_inputs.input_ids.to(_module_execution_device(self.text_encoder, _module_execution_device(self.unet)))
        self.empty_text_embed = self.text_encoder(text_input_ids)[0].to(self.dtype)


    @property
    def device(self) -> torch.device:
        return _module_execution_device(self.unet)
    ...
    def encode_rgb(self, rgb_in: torch.Tensor) -> torch.Tensor:
        ...
        """
        # encode
        rgb_in = rgb_in.to(device=_module_execution_device(self.vae, _module_execution_device(self.unet)), dtype=self.vae.dtype)
    ...
    def decode_depth(self, depth_latent: torch.Tensor) -> torch.Tensor:
        ...
        # decode
        depth_latent = depth_latent.to(device=_module_execution_device(self.vae, _module_execution_device(self.unet)), dtype=self.vae.dtype)
