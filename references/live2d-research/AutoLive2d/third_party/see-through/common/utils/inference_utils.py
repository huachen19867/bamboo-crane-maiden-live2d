...
def connected_component_split_part(tag, part_dict, min_area=64):
    mask = part_dict['img'][..., -1] > 10
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
        mask.astype(np.uint8), connectivity=8)
    if num_labels <= 2:
        return {tag: part_dict}
    ...
    tag2pinfo = {}
    for index, (label_id, _) in enumerate(components):
        cmask = labels == label_id
        stat = stats[label_id]
        img, depth, xyxy, depth_median = process_cuts(
            part_dict['img'],
            part_dict['depth'],
            part_dict['xyxy'],
            stat,
...
def seg_wcomponents_psd(srcp, target_tags, savep=None, min_area=64):
    part_infos = psd2partdicts(srcp)
    if savep is None:
        savep = osp.splitext(srcp)[0] + '_components.psd'
    if isinstance(target_tags, str):
        target_tags = target_tags.split(',')
    else:
        assert isinstance(target_tags, list)
    ...
    for tag in target_tags:
        ...
        if not np.any(mask):
            continue
        part_dict['mask'] = mask
        part_infos['parts'].update(connected_component_split_part(tag, part_dict, min_area=min_area))
