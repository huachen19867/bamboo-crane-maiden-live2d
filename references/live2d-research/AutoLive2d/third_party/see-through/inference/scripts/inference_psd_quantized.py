...
from utils.inference_utils import further_extr, seg_wdepth_psd, seg_wcomponents_psd
...
def build_marigold_pipeline(args):
    ...
    if quant_mode == 'none':
        ...
        if args.cpu_offload:
            # The custom Marigold pipeline calls VAE submodules directly in a few
            # places, which bypasses Diffusers' parent-module CPU offload hook.
            # Keep the small bf16 VAE on CUDA; NF4 still saves VRAM on the UNet.
            marigold_pipe.vae.to(device='cuda', dtype=torch.bfloat16)
            if not getattr(marigold_pipe.unet, 'is_quantized', False) and \
               not getattr(marigold_pipe.unet, 'quantization_method', None):
                marigold_pipe.unet.to(device='cuda', dtype=torch.bfloat16)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth_split_tags', type=str, default='',
                        help='comma-separated PSD layer names to split again by depth, for example: front hair,back hair,headwear')
    parser.add_argument('--component_split_tags', type=str, default='',
                        help='comma-separated PSD layer names to split by disconnected alpha components, for example: front hair')
    parser.add_argument('--component_min_area', type=int, default=64,
                        help='minimum connected-component area to keep when component splitting')
    ...
    if args.save_to_psd and args.component_split_tags.strip():
        print(f'Running component-based splitting: {args.component_split_tags}')
        psd_savep = osp.join(save_dir, f'{srcname}.psd')
        seg_wcomponents_psd(
            psd_savep,
            args.component_split_tags,
            savep=psd_savep,
            min_area=args.component_min_area,
        )
    if args.save_to_psd and args.depth_split_tags.strip():
        print(f'Running depth-based splitting: {args.depth_split_tags}')
        psd_savep = osp.join(save_dir, f'{srcname}.psd')
        seg_wdepth_psd(psd_savep, args.depth_split_tags, savep=psd_savep)
