...
**试验被证伪的路线（留档防重复）**：透明度键无法经 `EditArtMesh+Parameters` 写入（写 base）；网格顶点键形经自动化拖拽多次尝试均未记录（GetParameterKeys 证实 ArtUpperLidL/R 键形为空，眼睑平移必须人工 GUI 拖拽）。

**保存-重开复核（重要修正）**：把检查点关闭后重新打开，API 复核证实全部内容完整落盘：33 个 ArtMesh、778 顶点、8 层键位 [0,0.5] 齐全。同时修正一个错误结论——**参数超出键范围时会钳制到最近键**：重开后 L 侧 base=0 但参数 1 渲染正常（钳制到 key@0.5=100），R 侧结构相同因此参数 1 同样应为睁开；此前"R 眼在参数 1 隐形"的观察是临时驱动不触发重渲染造成的假象。base 值在键覆盖范围 [0,0.5] 外的参数段仅作兜底。保存时参数面板停在 左眼=1.0、右眼=0.0，编辑器重开右眼呈闭合属预期（键位 @0=0），拨回 1 即开。
